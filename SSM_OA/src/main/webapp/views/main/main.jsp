...
<head>
	...
	<script type="text/javascript">
		$(function () {
			//异步获取当前登录用户未完成的任务
			$.ajax({
				url:'${pageContext.request.contextPath}/task/getNotTask',
				type:'post',
				dataType:'json',
			...
			//异步获取最新的公告
			$.ajax({
				url:'${pageContext.request.contextPath}/notice/getNotList',
				type:'post',
				dataType:'json',
				success:function (data) {
					$.each(data,function (index,notice) {
						var nd=new Date(notice.ndate);
						var newdate=nd.getFullYear()+"-"+(nd.getMonth()+1)+"-"+nd.getDate();
						var ncon="<li class='ue-clear'>" +
								"<span>"+newdate+"</span>&nbsp;&nbsp;&nbsp;<a href='javascript:showWindow("+notice.nid+")' class='notice-title'>"+notice.ntitle+"</a>\n" +
								"</li>\n" +
								"<p>";
						$("#notices").append(ncon);
					})
			...
			$.ajax({
				...
				success:function (data) {
					var fomcon="";
					$.each(data,function (index,fom) {
						if(index%3==0){
							fomcon+="<tr bgcolor=\"#FFFFFF\">";
						}
						var fomtime=new Date(fom.createtime);
						var fdate=fomtime.getFullYear()+"-"+(fomtime.getMonth()+1)+"-"+fomtime.getDate();
						fomcon+="<td>" +
								"<ul class=\"notice-list\">\n" +
								"<li class=\"ue-clear\">\n" +
								"<span><img src=\"${pageContext.request.contextPath}/images/tx.png\" height=\"50px\" width=\"50px\"/></span> 发布时间："+fdate+
		...
		function showWindow(nid) {
			$.ajax({
				url:'${pageContext.request.contextPath}/notice/getTaskNid',
				type:'post',
				data:{"nid":nid},
				dataType:'json',
				success:function (data) {
			...
			<tr>
				...
				<td width="85%" valign="bottom"><div class='icoitem'>
					...
					<div class='icoitem'>
						...
						<div class='txt'>
							<a href='fom/getMyFomId'><u>我的帖子</u></a>
						</div>
					</div>
					<div class='icoitem'>
						<div class='ico'>
...
<table width="98%" align="center" border="0" cellpadding="3"
	...
	<tr bgcolor="#FFFFFF">
		<td>您有<a href="us/getMyTlist?st=-1"><font color="red" id="tcount">0</font></a>个任务未完成……&nbsp;
		</td>
	</tr>
</table>
